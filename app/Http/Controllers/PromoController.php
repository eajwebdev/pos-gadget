...
class PromoController extends Controller
{
    ...
    public function index(): Response
    {
        $promos = Promo::tableExists()
            ? Promo::with(['products:id,name', 'categories:id,name', 'creator:id,fname,lname'])
                ->latest()
                ->get()
    ...
    public function apply(Request $request): JsonResponse
    {
        ...
        if (! Promo::tableExists()) {
            return response()->json(['valid' => false, 'message' => 'Promos are not available until database migrations are run.'], 503);
        }
